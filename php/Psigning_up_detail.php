...
if(isset($_POST['json'])){
    ...
    if (!$result) {
        ...
        if(!$result2){echo "querry2 failed ".mysqli_error($conn);}
                ...
                $sql_insert_in_doctorDetail="INSERT INTO patient_detail(`username`,`email`,`id`,`phone_number`,`name`) values('$username','$email',$id,$phone_number ,JSON_OBJECT('firstname','".$name['firstname']."','lastname','".$name['lastname']."'))";
                ...
                if (!$result3) {echo "querry3 failed ".mysqli_error($conn);}
                    ...
                    echo $enc_obj;
                    $sql_insert_mainDetail="UPDATE patient_detail SET cradential_detail='$enc_obj' where id=$id and username='$username'";
